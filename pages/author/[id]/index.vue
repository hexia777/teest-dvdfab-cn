...
const getAuthorArticleList = async (author?: string) => {
  const params = {
    lang: locale,
    start: (Number(authorPage.value) - 1) * pageLen.value,
    len: pageLen.value,
  }
  ...
  if (Number(authorPage.value) === 1 && (!articleData?.list?.length || !productMap)) {
  ...
  // 判断是否是最后一页
  if (Number(authorPage.value) * pageLen.value >= articleData?.total || 0) {
    isEnd.value = true
  }
  ...
  authorPage.value = Number(authorPage.value) + 1
}
...
const handleScroll = () => {
  ...
  if (scrollHeight - (scrollTop + clientHeight) < 900 && !loading.value && !isEnd.value) {
    // 距离页面底部小于700像素，加载数据
    loadMoreData()
  }
}
...
<template>
  <div class="author-page">
    <div class="author-banner">
      <div class="container">
        <div class="photo-box author-detail">
    ...
    <div class="author-content">
      <BaseContainer>
        ...
        <h1 class="text-center mt80 mb40" v-html="localesMap.author_title_latest_posts"></h1>
        <ul class="flex flex-wrap gap30 mb60">
          <template v-for="index in Math.ceil(articleList.length / pageLen)" :key="index">
            <ul class="flex flex-wrap gap30 w100%">
        ...
        <div v-if="isEnd" class="text-center color-gray font-s-14">{{ useI18n().value.article.no_more }}</div>
        <div v-if="loading" class="text-center color-gray font-s-14">
          {{ useI18n().value.article.loading }}
        </div>
      </BaseContainer>
    </div>
  </div>
