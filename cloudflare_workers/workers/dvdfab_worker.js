...
async function updateModuleVersion(data) {
  return commonUpdate(VERSION_MODULE, data)
}
...
function getGlobalConfigJson() {
  return globalConfigJson
}

function getGlobalConfigTTl() {
  return globalConfigTTl
}
...
async function handleCache(req) {
  ...
  // 构造缓存 Key
  const moduleVersionJson = globalConfigJson[VERSION_MODULE] || {}
  const version = moduleVersionJson[urlType] || getCurrentFormattedTime()
  const newUrl = `${url}&_version=${urlType}_${version}`
