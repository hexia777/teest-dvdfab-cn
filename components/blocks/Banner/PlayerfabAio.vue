...
const props = defineProps({
  ...
  diffMedia: {
    ...
    default: () => {
      return {
        url: 'https://assets-test.dvdfab.cn/dvdfab/box/760.png',
      }
    },
  },
  // 是否展示 gift
...
const renderInfoHandle = (os: string, isClient = true) => {
  ...
  const freeData = getStrapiData(_curProInfo?.product_attrs?.data[0])
  let freeInfo = {}
  if (freeData?.type === 'gift-product') {
    freeInfo = {
      key: 'playerfab',
      name: freeData.tags[0].value,
      url: freeData.tags[1].value + '.htm',
      free: freeData.tags[2].value,
    }
  }
  bannerInfo.childPros.push(freeInfo)
  ...
  const extParams = ''
  // const extParams = getUpgradeExtParams(priceInfo, _curProInfo, 'playerfab', 'aio_banner')
  ...
  bannerInfo.btnInfo = {
    label: props?.paidText,
    href: '/player7.htm#upgrade_license',
...
<template>
  <div
    ...
    :class="[
      ...
      !commomStore.mobile ? (osText == 'win' ? 'aio-banner' : 'aio-banner') : '',
    ]"
  >
...
.aio-banner {
  background: #0d94f2 url('https://c6.dvdfab.cn/images/playerfab/banner_win.jpg') no-repeat 50%;
  background-size: cover;
  position: relative;
  z-index: 2;
}
.aio-mac-banner {
  background: #fe6a36 url('https://c6.dvdfab.cn/images/product/1x_m/en/passkey/banner_bg_mac.png') no-repeat
    50%;
  background-size: cover;
  position: relative;
  z-index: 2;
...
.product-bgm {
  background: linear-gradient(180deg, #104571, #0c1840);
}
</style>
